...
def T_target(pool: CandidatePool, kind: str) -> np.ndarray:
    ...
    for a, ws in enumerate(pool.orbit_data.weights):
        ...
        elif kind in {"odd-cat", "odd_cat"}:
            amp = 0 if not (w & 1) else omega**w / (2**((n-1)/2))
        else:
            raise ValueError("kind must be product, even_cat, or odd_cat")
...
def pair_polish_decomposition(
    pool: CandidatePool, target_span: np.ndarray, indices: Sequence[int], *,
    candidate_shortlist: int = 40, max_passes: int = 4, exact_tol: float = 1e-10,
    verbose: bool = False,
) -> tuple[np.ndarray, float]:
    """Deterministic two-coordinate local search.

    For each pair of selected positions, remove both atoms, rank all pool atoms by
    their *individual* gain relative to the remaining R-2 atoms, keep the best
    ``candidate_shortlist``, and exhaustively score all pairs from that shortlist.

    This is especially useful for small fixed-T instances where the one-coordinate
    heat-bath annealer can sit behind a genuine two-swap barrier.  It is intentionally
    separate from :func:`anneal_decomposition`, so callers can decide when the extra
    O(R^2 L^2) work is worthwhile.
    """
    C = pool.vectors
    T = orthonormalize_span(target_span)
    sel = np.asarray(indices, int).copy()
    rank = len(sel)
    N = C.shape[1]
    best_score = projection_score(C[:, sel], T)
    L = max(2, min(int(candidate_shortlist), N))
    for pas in range(max(1, int(max_passes))):
        improved = False
        for a in range(rank):
            for b in range(a+1, rank):
                mask = np.ones(rank, dtype=bool)
                mask[[a, b]] = False
                others = sel[mask]
                # Shortlist by single-atom gain in the quotient by span(others).
                cand = np.arange(N)
                one = _replacement_scores_direct(C, T, others, cand)
                one[np.isin(cand, others)] = -np.inf
                top = np.argsort(-one)[:L]
                ...
                for ii in range(len(top)):
                    ...
                    for jj in range(ii+1, len(top)):
                        y = int(top[jj])
                        if y in others or y == x:
                            continue
                        trial = np.concatenate([others, [x, y]])
                        sc = projection_score(C[:, trial], T)
                        if sc > local_best+1e-12:
                            local_best = sc
                            local_pair = (x, y)
                if local_pair is not None:
                    sel[a], sel[b] = local_pair
                    best_score = local_best
                    improved = True
                    if verbose:
                        print(
                            f"  pair-polish pass={pas} positions=({a},{b}) defect={T.shape[1]-best_score:.9g}")
                    if T.shape[1]-best_score < exact_tol:
                        return sel, float(best_score)
        if not improved:
            break
    return sel, float(best_score)
